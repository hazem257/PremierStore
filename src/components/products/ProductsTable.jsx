...
const ProductsTable = () => {
  ...
  const handleSaveEdit = () => {
    setProducts(products.map(p => 
      p.id === editingProduct.id ? { ...editedData } : p
    ));
  ...
  const handleAdd = () => {
    ...
    setNewProduct({ name: "", category: "", price: 0, stock: 0, sales: 0 });
  };
          ...
          <tbody className="divide-y divide-gray-700">
            {filteredProducts.map((product) => (
              ...
              >
                ...
                <td className="px-6 py-4 text-right text-sm text-gray-300">{product.stock}</td>
